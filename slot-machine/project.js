...
// 2. determing # of lines to bet on 
...
const getBet = (balance, lines) => {
    while (true) {
        const bet = prompt("Enter how much money you want to bet per line: ");
...
const spin = () => {
    ...
    for (let i = 0; i < COLS; i++) {
        reels.push([]);
        const reelSymbols = [...symbols]; // copy of available symbols
...
const game = () => {
    ...
    while (true) {
        ...
        const betAmount = getBet(balance, numLines);
        balance = balance - (betAmount * numLines);
        const reels = spin();
